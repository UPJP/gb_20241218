						...
						<c:forEach var="reply" items="${replylist }">
							<tr>
								...
								<td valign="top" style="padding-left: 10px;"><textarea
										...
										name="reply${reply.replynum }" id="reply${reply.replynum }" readonly>${reply.replycontents }</textarea> <br>
	
									<a href="javascript:updateReply(${reply.replynum },'${reply.password }')">[수정]</a> 
									&nbsp;&nbsp;&nbsp; 
									<a href="javascript:updateReadonly(${reply.replynum });">[수정 하기]</a>
									&nbsp;&nbsp;&nbsp; 
									<a href="javascript:delteReplay(${reply.replynum },'${reply.password }')">[삭제]</a></td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
...
<script >
   ...
   function delteReplay(replynum,password) {
 	let pw = prompt("댓글 비밀번호를 입력해주세요");
	   
	   if(pw == password){
		   document.replyForm.action
		   ="${pageContext.request.contextPath }/board/delteReplay.bo?replynum="+replynum;
