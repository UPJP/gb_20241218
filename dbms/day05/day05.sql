...
--inner join
-- 사원 이름, email 부서 아이디 부서명 job_title , job_id
...
--inner join
-- 사원 이름, email 부서 아이디 부서명 job_title , job_id
-- city 명칭을 추가 조회, 조건 : city가 Seattle인 도시만 조회
SELECT e.FIRST_NAME ,e.LAST_NAME, e.EMAIL , d.DEPARTMENT_ID ,d.DEPARTMENT_NAME  
		,e.JOB_ID,j.JOB_ID ,j.JOB_TITLE ,l.CITY
	FROM EMPLOYEES e 
	INNER JOIN DEPARTMENTS d 
	ON e.DEPARTMENT_ID =d.DEPARTMENT_ID 
	INNER JOIN JOBS j 
...
SELECT e.FIRST_NAME ,e.LAST_NAME ,e.HIRE_DATE,
	   jh.START_DATE ,jh.END_DATE, jh.JOB_ID ,e.EMPLOYEE_ID ,jh.EMPLOYEE_ID 
FROM EMPLOYEES e 
...
FROM EMPLOYEES e 
	...
	ON e.DEPARTMENT_ID = e2.MANAGER_ID 
...
WHERE SALARY < (SELECT AVG( SALARY) 
				FROM EMPLOYEES e ) 
;
-- 문제3) EMPLOYEES 테이블에서 (가장 많은 사원이 속해 있는 부서 번호)와 사원수를  출력하라.
SELECT MAX(COUNT(*))  
FROM EMPLOYEES e 
